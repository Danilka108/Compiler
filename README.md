...
1. (I.5) Построить РВ, описывающее номера карт, принадлежащих платежной системе Visa.

```
(\b4\d{3}-\d{4}-\d{4}-\d{4})|(\b4\d{3} \d{4} \d{4} \d{4})|(\b4\d{15})
```

![Тест](screenshots/reg_test_1.png)

2. (II.22) Построить РВ, описывающее положительные и отрицательные целые числа и числа с плавающей точкой (разделители запятая и точка).

```
[\+-]?[0-9]+(\.[0-9]+)?
```

![Тест](screenshots/reg_test_2.png)

3. (III.28) Построить РВ, описывающее однострочные комментарии (язык Python).

```
#.*$
```

![Тест](screenshots/reg_test_3.png)